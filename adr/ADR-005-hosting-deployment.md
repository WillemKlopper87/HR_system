...
## Decision
Docker Compose per environment (dev/staging/prod): `nginx → gunicorn/Django → PostgreSQL`, plus Celery worker + beat and Redis. Object storage as mounted volume (Azure Blob if cloud-hosted). Nightly `pg_dump` + off-host copy; quarterly restore rehearsal is the DR plan.
