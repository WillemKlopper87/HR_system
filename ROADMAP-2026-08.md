...
| **C1** 🔶 part 1 of 3 done 2026-08-19 (`b2ad0c0` + final-review fixes) | Establishment & lifecycle | Split into three separable pieces, each with its own spec (user's explicit decomposition). **① Position/establishment ✅ done** — new `establishment` app: post-numbered `Position` (approved vs filled, vacancy rate), configurable approval chain (`settings.POSITION_APPROVAL_CHAIN`) with a snapshotted audit trail, backfill of one approved post per serving employee, `Requisition.positions` M2M tying a vacancy to specific approved+vacant posts, `EmployeeVersion.position` (derived occupancy, DB-enforced single occupant), `/positions` page + multi-select picker on the requisition form (spec: `docs/superpowers/specs/2026-08-19-position-establishment-design.md`). **② `contract_end_date` / `probation_end_date` + reminders** (via H3/PC-1 engine) — not started, needs its own brainstorm/spec. **③ onboarding/offboarding checklists** (termination cascades: role assignments, liveness enrolment, collab account flag) — not started, needs its own brainstorm/spec | §7.2 #10 ✅ · #11 #12 | L (⅓ delivered) | H3 |
