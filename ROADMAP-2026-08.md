...
| **C1** 🔶 parts 1–2 of 3 done (part 1 2026-08-19 `b2ad0c0`; part 2 2026-08-20 `921f2a4` + final-review fixes) | Establishment & lifecycle | Split into three separable pieces, each with its own spec (user's explicit decomposition). **① Position/establishment ✅ done** — new `establishment` app: post-numbered `Position` (approved vs filled, vacancy rate), configurable approval chain (`settings.POSITION_APPROVAL_CHAIN`) with a snapshotted audit trail, backfill of one approved post per serving employee, `Requisition.positions` M2M tying a vacancy to specific approved+vacant posts, `EmployeeVersion.position` (derived occupancy, DB-enforced single occupant), `/positions` page + multi-select picker on the requisition form (spec: `docs/superpowers/specs/2026-08-19-position-establishment-design.md`). **② `contract_end_date` + renewal decisions ✅ done** — `EmployeeVersion.contract_end_date` (carried forward, surfaced when missing by a new `missing_contract_end_date` data-quality check instead of a backfill migration), `ContractRenewalDecision` recommend→decide workflow that executes in the same transaction via `apply_lifecycle_event` (renew / convert to permanent / let lapse, guarded to the current fixed-term version), a daily Celery expiry-reminder sweep with hr_admin escalation (`CONTRACT_REMINDER_OFFSETS_DAYS` / `CONTRACT_ESCALATION_DAYS`), and a `/contract-renewals` page (spec: `docs/superpowers/specs/2026-08-20-contract-end-date-tracking-design.md`). **`probation_end_date` was deliberately split out and is NOT built** — the user's own choice during brainstorming (that spec's §2); it is a related but distinct concern and still needs its own brainstorm/spec. **③ onboarding/offboarding checklists** (termination cascades: role assignments, liveness enrolment, collab account flag) — not started, needs its own brainstorm/spec | §7.2 #10 ✅ · #11 (contract half ✅, probation outstanding) #12 | L (⅔ delivered) | H3 |
