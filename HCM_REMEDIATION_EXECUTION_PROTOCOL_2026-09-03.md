...
| H-4 | Restricted identifiers and TOTP seeds lack app-layer encryption | MASTER | P1 | Static verified | DONE | 12 (rbac_audit) | 75d9663 (phase 1), 3a030ea (phase 2 cutover) | Full cutover, not a staged rollout — safe only because the user confirmed no live production data exists yet; this exact one-migration approach (backfill + drop plaintext + rename in a single deploy) must NOT be repeated once real employee data exists, per this document's own migration-safety section. Lookup HMAC fingerprint (rbac_audit.field_encryption.lookup_fingerprint) is implemented and tested but not wired to any field — no Restricted field is queried by exact match today. |
...
| R-1 | Primary-branch release baseline and required checks must be verified/enforced | REPO | P1 release | Verify current state | PARTIALLY DONE | 5 test files updated | 8ee9ae7, 7318dd6 | Postgres job root-caused by reproducing against a local postgres:17 container (Docker) matching CI's exact config: a known Django bug (ticket #35618, dup of #30448) where response.close() on a StreamingHttpResponse/FileResponse inside a TestCase kills the real connection via close_old_connections(); fixed via core_hr.test_utils.read_streaming_response() at all 5 affected call sites, verified with the full 1303-test suite green under `--parallel auto` against real Postgres. E2E job still has 4 pre-existing failures in performance.spec.ts/notifications.spec.ts, confirmed identical on a pre-remediation baseline commit — untouched, a frontend/E2E investigation outside this remediation's backend domains. |
