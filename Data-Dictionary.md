...
| compensation | pay_band (R, effective-dated), comp_proposal (R, since C6 also `proposal_type`/`cycle`/`exceeds_cycle_budget`/`baseline_salary_at_proposal`), benefits_election (S); since C6, also comp_cycle (I — name/dates/budget/department scope/status, no individual pay figure, not step-up-gated) | Sprint 10, C6 |
| assessments | assessment_assignment (S), assessment_result (S), provider_config (I) | Sprint 12 |
| ee_reporting | ee_snapshot (S, immutable), ee_report (S, versioned + sign-off chain ending at CEO/Accounting Officer), ee_plan (I — 5-yr sector targets + annual targets per level×group×gender + disability targets), ee_questionnaire (I — justifiable reasons, consultation, 24-category barriers/AA grid, monitoring), since C6, also ee_forum_member (S — `representation` can reveal union membership, redacted for the member carve-out), ee_forum_meeting (S — attendance M2M, content-sniffed minutes file, authenticated download), ee_plan_measure (I — per-BARRIER_CATEGORY barrier + AA measure with required owner + EEA13 time frame), ee_plan_progress_snapshot (S, immutable — frozen workforce/disability matrices + gaps vs annual targets / sector targets / EAP, suppressed per requester on read), and ee_plan gains `eap_profile`; all five carry a 60-month RETAIN retention rule (EE Regs 2025 reg. 9(15)); employer_config (I — Section A identity: DTI/PAYE/UIF/EE ref, SETA classification, EAP choice), **remuneration_record (R — per-employee annualised fixed + variable remuneration imported from SAP payroll; required to generate EEA4)** | Sprint 13 (spec: `EEA-Form-Spec-Notes.md`) |
